...
public class QueryData {
    private static QueryData instance; // 單例實例變數，用來保存唯一的 LoginData 物件
    ...
    public static QueryData getInstance() {
        if (instance == null) {  //檢查 instance 是否已經被建立
            //synchronized 鎖住關鍵字
            synchronized (QueryData.class) { // 在多執行緒（多線程）環境下，確保同一時間只有一個執行緒能夠進入這個區塊，從而保證只會產生一個唯一的 LoginData 實例（instance）
                if (instance == null) {
                    instance = new QueryData(); //如果還沒建立（instance == null）則建立一個新的。
                }
            }
        }
        return instance;
    ...
    public ArrayList<HashMap<String, String>> getQueryData() {
